...
# NOTE: no input normalization here on purpose. RVC's stock hubert_base
# checkpoint is the original fairseq HuBERT base model; the official
# pipeline (see infer/rtrvc.py's RVC.infer(), which this offline path
# mirrors) feeds it the raw waveform directly with no normalization step.
# An earlier version of this file added a do_normalize check via
# AutoFeatureExtractor, but assets/hubert_base has no
# preprocessor_config.json (it's a fairseq-converted checkpoint, not an
# HF one that ships that file), so the check just crashed. Removed to
# match the official pipeline exactly.
